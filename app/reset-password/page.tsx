    ...
    <div className="rounded-spotly border border-border bg-surface p-7 shadow-[0_18px_40px_rgba(67,53,47,0.1)]">
      {done ? (
        <>
          ...
          <button onClick={() => router.push("/login")} className="w-full rounded-full bg-terracotta py-2.5 text-sm font-semibold text-white">
            Back to login
          </button>
        </>
      ) : !token ? (
        <p className="text-sm text-error">
          ...
          <Link href="/forgot-password" className="font-semibold text-terracotta">request a new one</Link>.
        </p>
      ) : (
        <form onSubmit={handleSubmit}>
          ...
          <button type="submit" disabled={busy} className="w-full rounded-full bg-terracotta py-2.5 text-sm font-semibold text-white disabled:opacity-60">
    ...
    <div className="flex min-h-screen items-center justify-center px-5">
      <div className="w-full max-w-sm">
        <div className="mb-8 flex flex-col items-center text-center">
          <Logo height={48} />
          <p className="mt-2 text-sm text-warm-clay">Set a new admin password.</p>
        </div>
        <Suspense fallback={<div className="text-center text-warm-clay">Loading…</div>}>
          <ResetPasswordForm />
        </Suspense>
      </div>
    </div>
